...
const Login = ({ handleLogin = () => { } }) => {
 ...
 const [response, setResponse] = useState(null);
  ...
  const handleSubmit = async (e) => {
       e.preventDefault();
    try {
      const res = await axios.post("http://localhost:8080/newdb/login", formData);
  ...
  return (
    <div className='w-100 h-50 bg-green-400 mx-auto mt-20 rounded-lg'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 justify-center items-center h-full'>
        ...
        <button type="submit" className='bg-violet-800 hover:bg-violet-400 m-3 px-4 py-2 rounded-md text-white'>Login</button>
      </form>
    </div>
  )
}
